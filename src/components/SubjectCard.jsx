...
const SubjectCard = () => {
  const { promptData, addField, removeField, updatePrompt, addArrayItem } =
    usePrompt();

  // Ensure subject is an array
  const subjects = Array.isArray(promptData.details.subject)
    ? promptData.details.subject
    : [];
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-4 mb-4 shadow-lg">
      ...
      {subjects.map((subject, index) => (
        <div
          key={index}
          className="mb-6 border-l-2 border-zinc-700 pl-4 relative group"
        >
